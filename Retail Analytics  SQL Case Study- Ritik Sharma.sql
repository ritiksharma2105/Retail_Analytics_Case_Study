...
-- just checking with own --
select * from customer_profiles cp
Join Sales_transaction as st
on cp.CustomerID = st.CustomerID
Where cp.CustomerID is null; -- this is showing that there is no null value or missing value present
...
Update Sales_transaction st -- Updateing sales table  -- 91 = 91 What the price 100
Set Price = (
	Select pi.Price 
...
Where st.ProductID in
  ...
  -- Why do we need to use Sub-query?
...
3.4 Validation Step - Join
3.5 Compare values and check for the discrepency  -- JOIN and UPDATE and Sub Query 
*/

/* 3.6 -- Missing Values and Working on NULL
...
Select 
     ...
     ((Total_Sales - lag(Total_Sales) Over(order by Monthh)) /
     lag(Total_Sales) Over(order by Monthh)) * 100 as MoM_Growth
From Monthly_sales
...
Select 
    ...
    sum(QuantityPurchased*Price) as total_sales,
        Case 
           When Hour(transaction_datetime) between 0 and 5 Then "Late Night"
           When Hour(transaction_datetime) between 6 and 11 Then "Morning"
